...
def pressEinstellungen(btn):
	if btn=="Standardverzeichnis":
		standardverzeichnis=[]
		standardverzeichnis.append(str((bell.openBox("Standardverzeichnis auswählen",  fileTypes=[('Bibliothek', '*.bib')],parent="Einstellungen"))))
...
#Hilfe-Link
def HilfeLink(link):
	bell.infoBox("Was ist Isbn2BibTeX?", "Isbn2BibTeX ist ein Programm, welches entwickelt wurde, um die ISBN von Büchern einzuscannen und diese, samt Metadaten, automatisch dem Literaturverzeichnis einer LaTeX-Datei (BibTeX-Datei) hinzuzufügen.")
...
bell.addLabel("titel", "Willkommen bei ISBN2BibTeX!")
...
bell.addButton("Hilfe", pressEinstellungen)
bell.addButton("Standardverzeichnis", pressEinstellungen)
bell.addButton("Credits", pressEinstellungen)
bell.addButton("Scanner", pressEinstellungen)
bell.addNamedButton("Zurück", "ZurückEinstellungen", pressEinstellungen)
bell.stopSubWindow()
...
if 	scannerliste != []:
	for device in scannerliste:	
		try:
			...
			None
bell.setListBoxWidth("Scanner", 600)
...
#Metadaten finden und in Bibtex bei Variable metatext speichern
...
if metatext !=	"Fehler!":
	...
	app1=gui("erfolg")
	app1.hide("erfolg")
	app1.infoBox("Erfolgreich", "Ihr Buch wurde dem Literaturverzeichnis hinzugefügt!")
	exit()
	#app1.go()
else:
	app1=gui("fehler")
	app1.hide("fehler")
	app1.errorBox("Fehler", "Fehler! Keine Daten gefunden!")
	exit()
	#app1.go()
